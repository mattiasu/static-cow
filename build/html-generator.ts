...
function markdownToHtml(markdown: string): string {
  ...
  // Images — emit <picture> with WebP variants generated by optimize-images, original as fallback
  html = html.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (_match: string, alt: string, filename: string) => {
    const base = path.parse(filename).name;
    return `<figure class="article-image">
            <picture>
                <source type="image/webp" srcset="/assets/images/${base}-800w.webp 800w, /assets/images/${base}-400w.webp 400w" sizes="(max-width: 400px) 400px, 800px">
...
function generateArticles(posts: Post[], templatesDir: string, outputDir: string, siteUrl: string): void {
  ...
  posts.forEach(post => {
    ...
    const articleContent = renderTemplate(articleTemplate, {
      ...
      profileImageWebp: post.profileImage
        ? `/assets/images/${path.parse(post.profileImage).name}-48x48.webp`
        : '',
      intro: post.intro,
      content: htmlContent,
    });
